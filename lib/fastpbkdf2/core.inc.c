...
static inline void HMAC_INIT(_name)(HMAC_CTX(_name) * ctx, const uint8_t *key,
				    size_t nkey)
{
	/* Prepare key: */
	uint8_t k[_blocksz];
...
static inline void HMAC_UPDATE(_name)(HMAC_CTX(_name) * ctx, const void *data,
				      size_t ndata)
{
...
static inline void PBKDF2_F(_name)(const HMAC_CTX(_name) * startctx,
				   uint32_t counter, const uint8_t *salt,
				   size_t nsalt, uint32_t iterations,
				   uint8_t *out)
{
	uint8_t countbuf[4];
