...
static void sha1_raw_transform(const uint32_t state_in[5],
			       uint32_t state_out[5], const cf_sha1_block inp)
{
	uint32_t a = state_in[0], b = state_in[1], c = state_in[2],
		 d = state_in[3], e = state_in[4];
...
#define R0(v, w, x, y, z, i)                                                   \
	z += ((w & (x ^ y)) ^ y) + W[i] + 0x5a827999 + rotl32(v, 5);           \
	w = rotl32(w, 30)
#define R1(v, w, x, y, z, i)                                                   \
	z += (w ^ x ^ y) + W[i] + 0x6ed9eba1 + rotl32(v, 5);                   \
	w = rotl32(w, 30)
#define R2(v, w, x, y, z, i)                                                   \
	z += (((w | x) & y) | (w & x)) + W[i] + 0x8f1bbcdc + rotl32(v, 5);     \
	w = rotl32(w, 30)
#define R3(v, w, x, y, z, i)                                                   \
	z += (w ^ x ^ y) + W[i] + 0xca62c1d6 + rotl32(v, 5);                   \
	w = rotl32(w, 30)
...
static void sha1_convert_output(const uint32_t *restrict h,
...
static void cf_sha1_update(cf_sha1_context *ctx, const uint8_t *bytes,
			   size_t nbytes)
{
	cf_blockwise_accumulate(ctx->partial, &ctx->npartial,
				sizeof ctx->partial, bytes, nbytes,
...
static void cf_sha1_final(cf_sha1_context *ctx, uint8_t out[CF_SHA1_HASHSZ])
{
	...
	cf_blockwise_acc_pad(ctx->partial, &ctx->npartial, sizeof ctx->partial,
			     0x80, 0x00, 0x00, padbytes, sha1_update_block,
			     ctx);
