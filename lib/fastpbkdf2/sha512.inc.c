				 ...
				 const cf_sha512_block inp)
{
	...
	uint64_t a = state_in[0], b = state_in[1], c = state_in[2],
...
static void sha512_convert_input(cf_sha512_block inp64,
				 const uint8_t inp[CF_SHA512_BLOCKSZ])
{
...
static void cf_sha512_update(cf_sha512_context *ctx, const void *data,
			     size_t nbytes)
{
...
static void cf_sha512_final(cf_sha512_context *ctx,
			    uint8_t hash[CF_SHA512_HASHSZ])
{
	...
	size_t padbytes =
		CF_SHA512_BLOCKSZ - ((digested_bytes + 4) % CF_SHA512_BLOCKSZ);

	/* Hash 0x80 00 ... block first. */
	cf_blockwise_acc_pad(ctx->partial, &ctx->npartial, sizeof ctx->partial,
			     0x80, 0x00, 0x00, padbytes, sha512_update_block,
			     ctx);
